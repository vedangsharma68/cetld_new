-- Bind each payment idempotency key to the exact settlement request.
alter table public.payments
  alter column idempotency_key type text using idempotency_key::text,
  add column settle_remaining boolean not null default false;

drop function public.record_invoice_payment(uuid, uuid, numeric, uuid, text, boolean);
...
-- Create already-paid Assistant invoices and their payment history as one transaction.
-- The conflict branch also repairs rows created by the previous two-request implementation
-- only when they are exactly fully paid and have no payment history.
