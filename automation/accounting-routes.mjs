...
export async function handleAccountingRequest(request, response, dependencies = {}) {
  ...
  try {
    ...
    const integration = dependencies.integration || await import('./runtime.mjs').then(module => module.createAccountingRuntime({ env }));
    ...
    if (input.action === 'sync') return response.status(200).json(await integration.sync({ ...identity, provider, invoicePage:input.invoicePage ?? 1, paymentPage:input.paymentPage ?? 1 }));
